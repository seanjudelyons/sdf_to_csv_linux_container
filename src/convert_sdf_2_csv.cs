...
public class Convert
{
    public static void Main(string[] args)
    {
        ...
        SqlCeDataReader reader_content;
        ...
        foreach (string file in files)
        {
            ...
            foreach (DataRow dataRow_tableName in datatable_tableNames.Rows)
            {
                ...
                {
                    var outputFileName =
                        @"C:\windows\Microsoft.NET\Framework64\v4.0.30319\Output\"
                        + tableName
                        + ".csv";
                    ...
                    using (StreamWriter writetext = new StreamWriter(outputFileName))
                    {
                        ...
                        commandTableContent.CommandText = "SELECT * FROM " + tableName;

                        reader_content = commandTableContent.ExecuteReader();

                        while (reader_content.Read())
                        {
                            for (int i = 0; i < reader_content.FieldCount; i++)
                            {
                                if (i == (reader_content.FieldCount - 1))
                                {
                                    var tableContent = reader_content.GetValue(i);
                                    writetext.Write("\"");
                                    writetext.Write(tableContent);
                                    writetext.Write("\"");
                                }
                                else if (reader_content.GetValue(i) != DBNull.Value)
                                {
                                    var tableContent = reader_content.GetValue(i);
                                    writetext.Write("\"");
                                    writetext.Write(tableContent);
